...
def does_ship_fit_within_board_boundaries(board, coords_pair):
    ...
    x_e = coords_pair[1][0]
    y_e = coords_pair[1][1]
    max_x = len(board[0]) # our board is a rectangle
    max_y = len(board)
    return 0 <= x_b < max_x and 0 <= x_e < max_x and \
           0 <= y_b < max_y and 0 <= y_e < max_y
...
def is_ship_at_or_beside_another(used_area_points, ship_points_to_be_tested):
    for point_to_be_tested in ship_points_to_be_tested:
        ...
        # now, check if any candidate point is already used
